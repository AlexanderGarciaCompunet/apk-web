...
class StoreServerAPI {
  Future<http.Response> storeServerAPI() async {
    String path = "stores";
    Uri url = Uri.parse(ConfigEndpointsAccess.pathServerAccess + path);
    final token = HiveDB.getBoxUser().get("token");

    http.Response respHttp = await http.get(url, headers: {
    ...
    // MODO DEMO - Sin descifrado (comentado para demo)
